...
class BloodRelationsTrainer:
    ...
    def __init__(self, args):
        ...
        Your task is to correctly solve difficult multiple-choice questions (MCQs) from one of the following topics: "Blood relation", "Seating Arrangements", or "Truth-teller and Liar Problems".
        
        For each question, provide:
        1. A detailed step-by-step reasoning of how you arrived at the correct answer.
        2. The correct answer as a single capital letter: A, B, C, or D.
        
        Return the output strictly in the following JSON-like format:
        {{answer:"<correct_option>", reasoning:"<detailed_reasoning>"}}
        
        For example:
        {{answer:"D", reasoning:"C is the Engineer, so not the Lawyer. B is in Delhi and dislikes cricket; B is not the Doctor (Doctor is in Mumbai) and not the Lawyer (Lawyer is not in Kolkata, leaving Chennai, which is D’s only valid location). Hence, D is the Lawyer."}}
        
        Do **not** include any additional text, XML tags, explanations, or formatting outside this dictionary structure.
        """


        self.system_prompt2 = f"""
        You are an expert in logical reasoning and complex problem-solving.
        
        Your task is to GENERATE a difficult multiple-choice question (MCQ) from one of the following topics:
        - "Blood relation"
        ...
        1. The question must be challenging and require multi-step logical inference.
        2. Output must be in the exact format shown below, as a single Python dictionary.
        3. The dictionary must contain the following keys (use single quotes `'` only):
           - 'topic': The topic of the question.
           - 'question': A clearly written question.
           - 'choices': A list of 4 options in the format ['A) ...', 'B) ...', 'C) ...', 'D) ...'].
           - 'answer': A single uppercase letter ('A', 'B', 'C', or 'D') corresponding to the correct choice.
           - 'explanation': A step-by-step explanation of how to arrive at the correct answer.
        
        Strict formatting rules:
        - Use **only single quotes `'`** for all keys and values.
        - Do **not** include any XML tags, markdown, or extra text.
        - The output must be only the dictionary, no preamble or comments.
        
        Your goal is to create a **well-formed, difficult MCQ** that strictly follows the required format.
        """
    ...
    def _correctness_reward_func(self, prompts, completions, answer, **kwargs) -> List[float]:
        ...
        return [3.0 if r == a else -1.0 for r, a in zip(extracted_responses, answer)]
...
if __name__ == "__main__":
    ...
    1. SFT TRAINING:
       ...
       python -m trainer \
           --training_type sft \
           --mode train \
           --model_name "/jupyter-tutorial/AAIPL_129_212_191_47/ckpt/final_finetune_3" \
           --dataset_file /jupyter-tutorial/AAIPL_129_212_191_47/dataset/bb.json \
           --output_dir /jupyter-tutorial/AAIPL_129_212_191_47/ckpt/final_finetune_4 \
           --learning_rate 2e-5 \
           --num_train_epochs 3 \
    ...
    2. GRPO TRAINING:
       ...
       python -m trainer \
           ...
           --output_dir /jupyter-tutorial/AAIPL_129_212_191_47/ckpt/GRPO_answer_model \
